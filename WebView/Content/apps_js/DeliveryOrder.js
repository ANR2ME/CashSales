$(document).ready(function () {
    ...
    function ReloadGrid() {
        $("#list").setGridParam({ url: base_url + 'DeliveryOrder/GetList', postData: { filters: null }, page: 'first' }).trigger("reloadGrid");
    }

    function ReloadGridDetail() {
        $("#listdetail").setGridParam({ url: base_url + 'DeliveryOrder/GetListDetail?Id=' + $("#id").val(), postData: { filters: null }, page: 'first' }).trigger("reloadGrid");
    }
    ...
    $("#list").jqGrid({
        ...
        datatype: "json",
        colNames: ['ID', 'Code', 'SalesOrder Id', 'SalesOrder Code', 'DeliveryDate', 'Warehouse Id', 'Warehouse Name',
    ...
    $('#btn_add_detail').click(function () {
        ...
        if (id) {
            $.ajax({
                dataType: "json",
                url: base_url + "DeliveryOrder/GetInfo?Id=" + id,
    ...
    $('#btn_unconfirm').click(function () {
        ...
        if (id) {
            ...
            $.messager.confirm('Confirm', 'Are you sure you want to unconfirm record?', function (r) {
                if (r) {
                    $.ajax({
                        url: base_url + "DeliveryOrder/Unconfirm",
                        type: "POST",
    ...
    $('#delete_confirm_btn_submit').click(function () {
        ...
        $.ajax({
            url: base_url + "DeliveryOrder/Delete",
            type: "POST",
            contentType: "application/json",
            data: JSON.stringify({
    ...
    $("#form_btn_save").click(function () {
        ...
        if (id != undefined && id != '' && !isNaN(id) && id > 0) {
            submitURL = base_url + 'DeliveryOrder/Update';
        }
            // Insert
        else {
            submitURL = base_url + 'DeliveryOrder/Insert';
        }
    ...
    $("#listdetail").jqGrid({
        url: base_url,
        datatype: "json",
        colNames: ['Code', 'SalesOrder Detail Id', 'SalesOrder Detail Code', 'Item Id', 'Item Name', 'Quantity',
        ],
        colModel: [
                  ...
                  { name: 'salesorderdetail', index: 'salesorderdetail', width: 100, sortable: false },
    ...
    $('#btn_edit_detail').click(function () {
        ...
        if (id) {
            $.ajax({
                dataType: "json",
                url: base_url + "DeliveryOrder/GetInfoDetail?Id=" + id,
                success: function (result) {
                    if (result.Id == null) {
                        $.messager.alert('Information', 'Data Not Found...!!', 'info');
                    }
    ...
    $('#btn_del_detail').click(function () {
        ...
        if (id) {
            ...
            $.messager.confirm('Confirm', 'Are you sure you want to delete record?', function (r) {
                if (r) {
                    $.ajax({
                        url: base_url + "DeliveryOrder/DeleteDetail",
                        type: "POST",
                        contentType: "application/json",
                        data: JSON.stringify({
                            Id: id,
    ...
    $("#item_btn_submit").click(function () {
        ...
        // Update
        if (id != undefined && id != '' && !isNaN(id) && id > 0) {
            submitURL = base_url + 'DeliveryOrder/UpdateDetail';
        }
            // Insert
        else {
            submitURL = base_url + 'DeliveryOrder/InsertDetail';
        }
        ...
        $.ajax({
            ...
            url: submitURL,
            data: JSON.stringify({
                Id: id, DeliveryOrderId: $("#id").val(), SalesOrderDetailId: $("#SalesOrderDetailId").val(),
                ItemId: $("#ItemId").val(), Quantity: $("#Quantity").numberbox('getValue'),
            }),
            async: false,
            cache: false,
            timeout: 30000,
    ...
    $('#btnSalesOrder').click(function () {
        var lookUpURL = base_url + 'SalesOrder/GetListConfirmed';
        var lookupGrid = $('#lookup_table_salesorder');
        lookupGrid.setGridParam({
            url: lookUpURL
        }).trigger("reloadGrid");
        $('#lookup_div_salesorder').dialog('open');
    });

    jQuery("#lookup_table_salesorder").jqGrid({
        url: base_url,
        datatype: "json",
        mtype: 'GET',
    ...
    $("#lookup_table_salesorder").jqGrid('navGrid', '#lookup_toolbar_salesorder', { del: false, add: false, edit: false, search: true })
           .jqGrid('filterToolbar', { stringResult: true, searchOnEnter: true });

    // Cancel or CLose
    $('#lookup_btn_cancel_salesorder').click(function () {
        $('#lookup_div_salesorder').dialog('close');
    });

    // ADD or Select Data
    $('#lookup_btn_add_salesorder').click(function () {
        var id = jQuery("#lookup_table_salesorder").jqGrid('getGridParam', 'selrow');
        if (id) {
            var ret = jQuery("#lookup_table_salesorder").jqGrid('getRowData', id);

            $('#SalesOrderId').val(ret.id).data("kode", id);
            $('#SalesOrder').val(ret.code);
            $('#lookup_div_salesorder').dialog('close');
        } else {
            $.messager.alert('Information', 'Please Select Data...!!', 'info');
        };
    ...
    $('#btnItem').click(function () {
        var lookUpURL = base_url + 'SalesOrder/GetListDetail?Id=' + $("#SalesOrderId").val();
